...
// to checkout the cart
	public Checkoutpage clickcheckout() throws InterruptedException {
		 ...
		 * "(//button[@aria-label='Close'])[2]]"); element.click(); try {
		 * findElement("Xpath", "(//*[local-name()='svg'])[69]").click();
		 * 
		 * }catch(Exception e) {
		 * 
		 * }
	...
	public Checkoutpage customersignin() {
	  ...
	  findElement("Xpath", "//button[text()='Continue as Guest']").click();
		/*
		 * findElement("id", "fld-e").sendKeys("kartika2611@gmail.com"); findElement("id",
	...
	//order details
	public Checkoutpage gettingorder() {
		
		findElement("id", "user.emailAddress").sendKeys("kartika2611@gmail.com");
		findElement("id", "user.phone").sendKeys("8996533698");
		findElement("Xpath", "//button[@class='btn btn-lg btn-block btn-secondary']").click();
		return this;
	}
	 
	//it had some issue because of popup appear so comment it
	  //customer delivery details 
/*	public Checkoutpage deliverydetails() {
	  
	  findElement("id", "firstName").sendKeys("kartika");
	  
	  findElement("id", "lastName").sendKeys("Hari");
	  
	  findElement("id", "street").sendKeys("19th St");
	  ...
	  findElement("Xpath",
	  "//span[text()='Continue to Schedule Delivery']").click();
	...
	public Checkoutpage datepick() {
	  ...
	  String expectedMonth = "August"; // get the month
	  
	  int selectDate = 15; // get the date
	  
	  while (true) { // verify the actual month is equals to expected month 
		  
		  String  actualMonth =   driver.findElement(By.xpath("/caption[text()='August 2024']")).getText();
	  ...
	  driver.findElement(By.xpath("(//span[@class='sr-only'])[9]")).click(); }
	  
	  } // click the selected date
	  driver.findElement(By.xpath("//table//tr//td[//span[text()='15']")).click();
	  
	  // print the selected date String date =
	  driver.findElement(By.xpath("[//span[text()='15']")).getText();
	  ...
	  Select select = new Select(selectele); select.selectByIndex(4);
	  
	  WebElement year = findElement("id", "expYear");
	  
	  Select yearselect = new Select(year); yearselect.selectByIndex(2);
	  
	  findElement("id", "cvv").sendKeys("123");
	  
	  findElement("id", "first-name").sendKeys("Hari");
	  
	  findElement("id", "last-name").sendKeys("Kartika");
